...
class AsyncCountryStateCity:
    ...
    async def request(
        ...
        """
        request_path(path)
        return await self._send(Endpoint(path, request_params(params)))
    ...
    async def get_postcodes_by_code(
        ...
        result = cast(
            JsonDict,
            await self._data(_endpoints.postcode(country, code, fields=fields)),
        )
        return cast(List[Postcode], result.get("data", []))

    async def get_postcodes_of_country(
        self,
        country: str,
        *,
        q: Optional[str] = None,
